// best (forward only): EvoAttention forward kernel selected as the
// winner of the experiments/ sweep. Identical to v6_one_sem.cu — see
// log.md for the path that got us here.
// One wait() per inner-loop iteration instead of four.
...
void evo_fwd_ker(const __grid_constant__ evo_fwd_globals<D> g) {
    ...
    __shared__ kittens::semaphore load_arrived[K::stages];   // pools K+V+pb+rm
