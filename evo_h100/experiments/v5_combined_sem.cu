// v5_combined_sem: v4 with pb and rm sharing one semaphore.
// One fewer wait() per inner-loop iteration.
...
void evo_fwd_ker(const __grid_constant__ evo_fwd_globals<D> g) {
    ...
    // pb + rm share one semaphore now (they both gate softmax).
    __shared__ kittens::semaphore pbrm_smem_arrived[K::stages];
    ...
    if (threadIdx.x == 0) {
        ...
        for (int j = 0; j < K::stages; j++) {
            ...
            init_semaphore(pbrm_smem_arrived[j], 0, 1);
        ...
        for (int j = 0; j < K::stages - 1; j++) {
            ...
            tma::expect_bytes(pbrm_smem_arrived[j], sizeof(pb_tile) * CW + sizeof(rm_vec));
            for (int wg = 0; wg < CW; wg++) {
                coord<pb_tile> pb_tile_idx = {batch_idx, head_idx, seq_idx + wg, j};
                tma::load_async(pb_smem[wg][j], g.pb, pb_tile_idx, pbrm_smem_arrived[j]);
            }
            coord<rm_vec> rm_idx = {batch_msa_idx, 0, 0, j};
            tma::load_async(rm_smem[j], g.rm, rm_idx, pbrm_smem_arrived[j]);
        }
    }
    __syncthreads();
    ...
    if (warpgroupid == NUM_WARPGROUPS - 1) {
        ...
        if (warpid == NUM_WORKERS - 4) {
            for (int kv_idx = pipe_idx - 1; kv_idx <= kv_iters; kv_idx++) {
                ...
                warp::tma::expect_bytes(pbrm_smem_arrived[s], sizeof(pb_tile) * CW + sizeof(rm_vec));
                for (int wg = 0; wg < CW; wg++) {
                    coord<pb_tile> pb_tile_idx = {batch_idx, head_idx, seq_idx + wg, nxt};
                    warp::tma::load_async(pb_smem[wg][s], g.pb, pb_tile_idx, pbrm_smem_arrived[s]);
                }
                coord<rm_vec> rm_idx = {batch_msa_idx, 0, 0, nxt};
                warp::tma::load_async(rm_smem[s], g.rm, rm_idx, pbrm_smem_arrived[s]);
        ...
        for (int kv_idx = 0; kv_idx <= kv_iters; kv_idx++) {
            ...
            wait(pbrm_smem_arrived[s], phase);
