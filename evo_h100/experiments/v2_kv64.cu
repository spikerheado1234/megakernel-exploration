// v2_kv64: identical to v1, except kv_height=64 (instead of 128).
// Smaller att_block / pb_reg register tiles (16x64 fp32 = 32 regs/thread)
// vs (16x128 fp32 = 64 regs/thread). Hypothesis: less register pressure
// → smaller spill, lower softmax cost per iter, more room for pipelining.
...
template<> struct evo_fwd_tile_dims<64> {
    ...
    constexpr static int kv_height           = 64;   // <-- 128 -> 64
    constexpr static int consumer_warpgroups = 2;
    constexpr static int stages              = 4;    // <-- 3 -> 4
};
