// v10_overlap_load: v6 layout, but pb / rm shared-memory loads and the
// LOG2E scaling are moved BEFORE mma_async_wait so they overlap with the
// QK^T WGMMA on the tensor cores. Hypothesis: the pb_reg load is a swizzled
// smem read; the LOG2E mul is a CUDA-core op. Both can run in parallel
// with the in-flight WGMMA, hiding their latency.
...
void evo_fwd_ker(const __grid_constant__ evo_fwd_globals<D> g) {
    ...
    if (warpgroupid == NUM_WARPGROUPS - 1) {
        ...
        for (int kv_idx = 0; kv_idx <= kv_iters; kv_idx++) {
            ...
            // ---- Overlap with QK MMA: pb / rm load + LOG2E scaling ----
