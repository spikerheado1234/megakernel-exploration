"""torch.autograd.Function wrapper around the TK EvoAttention kernel.

Forward + backward (production module `_C`). The backward dispatches between an
mma.sync kernel (seq_len <= 512) and a WGMMA kernel (seq_len > 512) inside the
C++ wrapper; the Python side is unchanged.
